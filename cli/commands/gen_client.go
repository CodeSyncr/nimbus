...
type RouteManifestEntry struct {
	...
	Response *ResponseManifestType       `json:"response,omitempty"`
}
...
// ResponseManifestType describes the structural type representation of a Go type.
type ResponseManifestType struct {
	Kind       string                          `json:"kind"`
	Type       string                          `json:"type,omitempty"`
	Fields     map[string]ResponseManifestType `json:"fields,omitempty"`
	Elem       *ResponseManifestType           `json:"elem,omitempty"`
	IsNullable bool                            `json:"is_nullable,omitempty"`
}

func toTSType(t *ResponseManifestType) string {
	if t == nil {
		return "any"
	}
	...
	case "array":
		if t.Elem != nil {
			baseType = fmt.Sprintf("Array<%s>", toTSType(t.Elem))
		} else {
			baseType = "any[]"
		}
	case "map":
		if t.Elem != nil {
			baseType = fmt.Sprintf("Record<string, %s>", toTSType(t.Elem))
		} else {
	...
	case "struct":
		if len(t.Fields) == 0 {
			...
			for _, k := range keys {
				...
				// Format fields, adding indentation
				fieldTS := toTSType(&field)
				// replace newlines to indent nested structs
				indentedFieldTS := strings.ReplaceAll(fieldTS, "\n", "\n  ")
				sb.WriteString(fmt.Sprintf("  %s%s: %s;\n", k, optionalSign, indentedFieldTS))
			}
			sb.WriteString("}")
			baseType = sb.String()
		}
...
// writeDataDTS writes interface declarations for validation schemas and response types.
...
func (c *GenClientCommand) writeRegistry(outDir string, entries []RouteManifestEntry) error {
	...
	for _, e := range entries {
		...
		if e.Response != nil {
			id := routeNameToIdentifier(e.Name)
			typeResponse = fmt.Sprintf("Data.%sResponse", id)
		} else {
			typeResponse = "any"
		}
